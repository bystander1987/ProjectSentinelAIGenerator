...
def create_role_prompt(role: str, topic: str, context: Optional[str] = None) -> str:
    ...
    if context:
        context_prompt = f"""
    次の参考文書を最も重要な情報源、かつ唯一の情報源として厳密に扱ってください。この文書に記載されている情報は最優先事項であり、必ず議論の中心に置き、具体的な内容に言及してください。
    ...
    最優先指示: 
    1. あなたの役割と参考文書の内容に基づいて、トピック「{topic}」について議論してください。参考文書に記載されていない情報は一切使用しないでください。
    2. 参考文書の内容を唯一の情報源として発言を組み立ててください。
