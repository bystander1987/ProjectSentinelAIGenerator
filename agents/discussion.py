...
def agent_response(
    ...
    # アップロードされた文書に関する指示を追加
    if context:
        prompt_with_context += """

    絶対に遵守すべき最重要ルール:
    - アップロードされた文書に記載されている内容のみについて発言してください。
    - 文書に記載されていない情報や知識に基づいた発言は一切禁止です。
    - この文書に明示的に記載されていない限り、一般的な知識や外部情報を用いた発言は絶対に行わないでください。
