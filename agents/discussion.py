...
def create_role_prompt(role: str, topic: str, context: Optional[str] = None) -> str:
    ...
    重要: 
    - アップロードされた資料の内容を最も重要な情報源として優先的に参照し、発言してください。
    - 資料に記載されている事実、数値、提案に基づいて発言を行ってください。
    ...
    あなたの役割と参考文書の内容に基づいて、トピック「{topic}」について議論してください。参考文書の内容を最優先事項として発言を組み立ててください。参考文書に記載がない内容よりも、参考文書に記載されている内容を常に優先してください。
    """
...
def agent_response(
    ...
    重要:
    - アップロードされた資料の内容を最も重要な情報源として優先的に参照してください。
    - 資料に記載されている事実、数値、提案に基づいて発言を行ってください。
