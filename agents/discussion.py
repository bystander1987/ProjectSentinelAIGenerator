...
def create_role_prompt(role: str, topic: str, context: Optional[str] = None) -> str:
    ...
    最優先指示:
    - アップロードされた文書の内容を最も重要な情報源として扱ってください。文書に記載の情報は他のすべての情報源よりも優先されます。
    - 文書に記載されている具体的な事実、数値、データ、見解を積極的に引用して発言してください。
    
    重要: 
    - 他の参加者の発言内容の具体的な箇所を指摘し、具体的な言葉を引用しながら議論を進めてください。
    - 自分の意見や主張の根拠は必ずアップロードされた文書から引用してください。
    - 具体的なデータや引用がない発言は避けてください。常に文書の内容に基づいた発言を心がけてください。
    ...
    if context:
        context_prompt = f"""
    次の参考文書を最も重要な情報源として扱ってください。この文書に記載されている情報は最優先事項であり、必ず議論の中心に置き、具体的な内容に言及してください。
    
    発言には必ず参考文書から少なくとも1つ以上の具体的な情報を引用してください。参考文書の事実、数値、分析結果を重視して議論を展開してください。
    文書に記載されていない情報よりも、文書に記載されている情報を常に優先してください。
    
    参考文書:
    {context}
    
    最優先指示: 
    1. あなたの役割と参考文書の内容に基づいて、トピック「{topic}」について議論してください。
    2. 参考文書の内容を最優先事項として発言を組み立ててください。
    3. 文書に記載されていない内容を述べるよりも、文書に記載されている内容を直接引用してください。
    4. 発言には必ず文書から直接引用した内容とその情報がどのように重要かを説明してください。
    """
...
def agent_response(
    ...
    最優先指示:
    - アップロードされた文書の内容を最も重要な情報源として扱い、必ず言及してください。この文書の情報は他のすべての情報源よりも優先されます。
    - 文書に記載されている具体的な事実、数値、データ、見解を積極的に引用して発言してください。
    - 自分の意見や主張の根拠は必ずアップロードされた文書から引用してください。文書の内容に基づかない発言は避けてください。
...
def generate_discussion(
    ...
    try:
        ...
        if vector_store:
            ...
            discussion.append({
                "role": "システム",
                "content": f"文書分析が完了しました。この内容を踏まえて、テーマ「{topic}」についての議論を開始します。常に文書の内容を最も重要な情報源として優先してください。"
            })
